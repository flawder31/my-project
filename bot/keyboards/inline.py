...
# IMPROVED: added payment history button to main menu
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="\U0001f4b0 История платежей", callback_data="payment_history")],
...
# IMPROVED: explicit confirmation step before payment
def confirm_payment_kb(tariff_id: int, amount: int, promo_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"\u2b50 Подтвердить и оплатить {amount} Stars",
...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="\U0001f4b0 Платежи", callback_data="admin_payments:0")],
...
def admin_payments_pagination_kb(page: int, has_next: bool) -> InlineKeyboardMarkup:
    buttons = []
    if page > 0:
        buttons.append(
            InlineKeyboardButton(text="\u2b05\ufe0f Назад", callback_data=f"admin_payments:{page - 1}")
        )
    if has_next:
        buttons.append(
            InlineKeyboardButton(text="Вперёд \u27a1\ufe0f", callback_data=f"admin_payments:{page + 1}")
        )
    rows = []
    if buttons:
        rows.append(buttons)
    rows.append([InlineKeyboardButton(text="\U0001f6e0 Админ-меню", callback_data="admin_menu")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
